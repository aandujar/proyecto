<template>
  ...
  <select v-model="provincia" class="listas">
    <option disabled value="">Provincias</option>
    <option v-for="prov in provincias" :key="prov" :value="prov">{{prov}}</option>
  </select>

  <select v-model="localidadAlicante" class="listas" v-if="mostrarAlicante">
    <option disabled value="">Localidades</option>
    <option v-for="localidad in localidadesAlicante" :key="localidad" :value="localidad">{{localidad}}</option>
  </select>

  <select v-model="localidadCastellon" class="listas" v-if="mostrarCastellon">
    <option disabled value="">Localidades</option>
    <option v-for="localidad in localidadesCastellon" :key="localidad" :value="localidad">{{localidad}}</option>
  </select>

  <select v-model="localidadValencia" class="listas" v-if="mostrarValencia">
    <option disabled value="">Localidades</option>
    <option v-for="localidad in localidadesValencia" :key="localidad" :value="localidad">{{localidad}}</option>
  </select>
 ...
 <select v-model="deporte" class="listas">
    <option disabled value="">Deportes</option>
    <option v-for="deporteLista in deportes" :key="deporteLista" :value="deporteLista">{{deporteLista}}</option>
  </select>
  ...
  export default {
    ...
    watch: {
      ...
      hora: function(){
        var hour = this.hora.split(":");
         if((hour.length!=2) || (isNaN(hour[0])) || (hour[0]>23) || (hour[0]<0) || (hour[0].length!=2) || (isNaN(hour[1])) || (hour[1]>59) || (hour[1]<0) || (hour[1].length!=2)){
            this.errorHora = true;
          }else{
    ...
    computed: {
      descripcionErrors () {
        ...
        !this.$v.descripcion.maxLength && errors.push('La descripción no puede sobrepasar los 200 caracteres')
        !this.$v.descripcion.required && errors.push('La descripción es obligatoria.')
        return errors
      },
      participantesErrors () {
        const errors = []
        if (!this.$v.participantes.$dirty) return errors
        !this.$v.participantes.between && errors.push('El número máximo de participantes debe ser entre 2 y 100')
        !this.$v.participantes.required && errors.push('Es obligatorio indicar el número de participantes.')
        return errors
      }
    },
        ...
        if(this.provincia!=''){
          ...
          if(this.hora!='' && this.errorHora==false){
            ...
            if(errorAjax==false){
              ...
              axios.post('http://localhost:3002/crearEvento', {
                data:{
                          id: id,
